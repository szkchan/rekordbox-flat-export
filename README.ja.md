...
| 検証状況 | **CDJ-400 実機での再生を確認済み** | 実機は未検証。スキーマ通りの合成データでの読み込みロジックの動作は確認済み |
...
`export.pdb` 側は CDJ-400 実機での再生を確認済みです。One Library (ベータ) はまだ実機確認をしていないため、
まずは **ドライラン機能**（実際にはコピーせず、何がコピーされるかログだけ表示する機能）で
少数のプレイリストを確認してから、実際のコピーに進むことをおすすめします。
